...
if (isset($_GET['delete_id']) && isset($_GET['type'])) {
    ...
    $table = $_GET['type'] === 'regular' ? 'tbl_regulars' : 'tbl_students';

    // Get uid before deleting
    $uidResult = mysqli_query($conn, "SELECT uid FROM $table WHERE id = $deleteId");
    $uidRow = $uidResult ? mysqli_fetch_assoc($uidResult) : null;
    $uid = $uidRow ? $uidRow['uid'] : null;

    // Delete from the respective table
    $deleteQuery = "DELETE FROM $table WHERE id = $deleteId";
    if (mysqli_query($conn, $deleteQuery)) {
        // If uid exists, set inuse=0 in tbl_rfid_auth
        if ($uid) {
            mysqli_query($conn, "UPDATE tbl_rfid_auth SET inuse = 0 WHERE uid = '" . mysqli_real_escape_string($conn, $uid) . "'");
        }
        $statusMsg = '<div class="alert alert-success">✅ User deleted successfully and RFID marked as not in use!</div>';
    } else {
                          ...
                          <?php while ($row = mysqli_fetch_assoc($studentResult)): ?>
                            <tr>
                              ...
                              <td class="action-icons">
                                ...
                                <?php endif; ?>
                                <a href="?delete_id=<?= $row['id']; ?>&type=<?= $row['source']; ?>" 
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <form method="POST" action="">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
            <button type="submit" class="btn btn-primary">Update Regular User</button>
          </div>
        </form>
      </div>
    </div>
